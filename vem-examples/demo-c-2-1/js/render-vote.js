...
(function(){
  ...
  var pre = function(){
    ...
    $('#vem-rate-item-2').html(vem.info.db[vem.api.target.id()][2]);  // target description
    $('#vem-rate-item-3').html(vem.info.db[vem.api.target.id()][1]);  // target group
    $('#vem-rate-item-4').html(vem.config.max_tags);                  // max tags
